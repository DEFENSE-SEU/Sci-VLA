...
def _replace_execute_body(template_code: str, execute_body_code: str) -> str:
...
- Main sequence: obstacle clearance -> move to target qpos.
...
The final step must be resuming the target pose: moving directly to the desired target pose via interpolation.
...
{{
    "plan_steps": [
        ...
        ...
        "step n: move...",
        "step n+1: move to target qpos",
        "step n+2: change to target gripper state"
    ],
    "safety_notes": ["...", "..."],
...
Task:
Please generate the corresponding instruction code based on the steps defined in the plan JSON file.

Planning list (must be followed):
{plan_steps_json}

Allowed APIs inside execute body:
{allowed_apis_json}
...
4. End with moving to the provided target joint position using move_to_target_qpos.
'''
    ...
    code = _replace_execute_body(template_code, execute_body)
