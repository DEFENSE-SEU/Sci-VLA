...
class ThermalCyclerManipulate(Task):
    default_scene = SCENE_ROOT / "thermal_cycler_tasks.xml"
    # default_task = "open_thermal_cycler_lid"
    # default_task = "place_pcrPlate_into_thermalCycler"
    # default_task = "close_thermal_cycler_lid"
    # default_task = "screw_tighten_knob"
    default_task = "press_thermal_cycler_button"
    # default_task = "screw_loosen_knob"
    # default_task = "take_pcrPlate_from_thermalCycler"
    ...
    def reset(self, seed: int | None = None):
        ...
        match self.task:
            ...
            case "thermal_cycler_long_task_1":
                ...
                prefix=''
            case "thermal_cycler_long_task_2":
            ...
            case "screw_tighten_knob":
                ...
                self.data.qpos[self.instrument.lever_qposadr] = self.instrument.lever_jntlimit[0]
                mujoco.mj_kinematics(self.model, self.data)
                prefix = 'screw tighten the knob of the thermal cycler'
...
class ThermalCyclerManipulateExpert(ThermalCyclerManipulate, Expert):
    ...
    def execute(self):
        ...
        if self.task == 'close_thermal_cycler_lid':
            ...
            terminal_pose = Pose(grip_pose.pos+(0.0, 0, 0.1), grip_pose.quat)
            path = self.interpolate(cur_pose, terminal_pose, 6)
            self.path_follow(path)
            cur_pose = self.arm.get_site_pose(self.data)
            terminal_pose = Pose(plate_pos+(0.0, 0, 0.001), cur_pose.quat)
            ...
            self.path_follow(path)
            cur_pose = self.arm.get_site_pose(self.data)
            terminal_pose = Pose(self.pcrPlate_pos+np.array([0.0, 0.0, 0.1]), grip_pose.quat)
            path = self.interpolate(cur_pose, terminal_pose, 20)
            self.path_follow(path)
            cur_pose = self.arm.get_site_pose(self.data)
            ...
            self.path_follow(path)
            cur_pose = self.arm.get_site_pose(self.data)
            terminal_pose = Pose(self.pcrPlate_target_pos+np.array([0.0, 0.0, 0.01]), cur_pose.quat)
            path = self.interpolate(cur_pose, terminal_pose, 20)
            self.path_follow(path)
            self.gripper_control(0)
            cur_pose = self.arm.get_site_pose(self.data)
