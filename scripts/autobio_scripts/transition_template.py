...
class TransitionExpert:
    ...
    def execute(self):
        ...
        # step 1: open gripper to release the green rack
        self.gripper_control(0, delay=300)

        # step 2: move EE 10cm along +z axis to lift above machine lid
        ...
        # step 3: move EE 15cm along +y axis to retract towards robot base
        cur_pose = self.get_site_pose(self.data)
        end_pose = Pose(pos=cur_pose.pos + (0.0, 0.15, 0.0), quat=cur_pose.quat)
        path = self.interpolate(cur_pose, end_pose, 100)
        self.path_follow(path)

        # step 4: rotate EE -45 degrees around x-axis to orient for home position
        cur_pose = self.get_site_pose(self.data)
        target_quat = self.rotate_gripper(-45, 'x', cur_pose.quat)
        end_pose = Pose(pos=cur_pose.pos, quat=target_quat)
        path = self.interpolate(cur_pose, end_pose, 100)
        self.path_follow(path)

        # step 5: move EE 10cm along -x axis to clear machine depth
        cur_pose = self.get_site_pose(self.data)
